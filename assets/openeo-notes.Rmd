...
To use each of them a user has to learn a dedicated APIs.
...
frome one backe-end to another and expect to get similar results with little 
...
DT::datatable(info, options = list(pageLength = 6), rownames = F)
```
...
names(info) = c("Name", "Spatial Extent", "Starting from")
DT::datatable(info, options = list(pageLength = 4), rownames = F)
```
---
...
```{r eval = F}
# apply a transformation to visualize data as png
